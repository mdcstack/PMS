...
function GetLeaveDetails($emp_code, $month) {
    return GetEmployeeLWPDataByEmpCodeAndMonth($emp_code, $month);
}

function GetSalaryHeads($emp_code, $month) {
    $salaryData = GetEmployeeSalaryByEmpCodeAndMonth($emp_code, $month);
    if (!empty($salaryData)) {
        return $salaryData;
    }
    return GetEmployeePayheadsByEmpCode($emp_code);
}

/**
 * Calculate hours worked between punch in and punch out times
 * @param string $punch_in
 * @param string $punch_out
 * @return float
 */
function calculate_hours_worked($punch_in, $punch_out) {
    $in = strtotime($punch_in);
    $out = strtotime($punch_out);
    if ($in && $out && $out > $in) {
        $hours = ($out - $in) / 3600; // seconds to hours
...
/**
 * Assign all payheads to all employees who are missing any payhead.
 * This ensures every employee has every payhead in wy_pay_structure.
 */
function assign_all_payheads_to_all_employees() {
    global $db;
    // Get all employees
    $employees = mysqli_query($db, "SELECT emp_code FROM " . DB_PREFIX . "employees");
    if ($employees && mysqli_num_rows($employees) > 0) {
        ...
        if ($payheads_query && mysqli_num_rows($payheads_query) > 0) {
            while ($ph = mysqli_fetch_assoc($payheads_query)) {
                $payheads[] = $ph['payhead_id'];
            }
        }
        // For each employee, check and insert missing payheads
        while ($emp = mysqli_fetch_assoc($employees)) {
            $emp_code = $emp['emp_code'];
            // Get payheads already assigned
            $assigned = [];
            $assigned_query = mysqli_query($db, "SELECT payhead_id FROM " . DB_PREFIX . "pay_structure WHERE emp_code = '$emp_code'");
            if ($assigned_query && mysqli_num_rows($assigned_query) > 0) {
                while ($a = mysqli_fetch_assoc($assigned_query)) {
                    $assigned[] = $a['payhead_id'];
                }
            }
            // Insert missing payheads
            foreach ($payheads as $ph_id) {
                if (!in_array($ph_id, $assigned)) {
                    mysqli_query($db, "INSERT INTO " . DB_PREFIX . "pay_structure (emp_code, payhead_id, default_salary) VALUES ('$emp_code', $ph_id, 0)");
                }
            }
        }
    }
